...
def paginate_issues(owner: str, repo: str, token: str, query: Dict[str, str], limit: int) -> List[Dict]:
...
def format_row(item: Dict) -> str:
    ...
    labels = ','.join([lbl.get('name') for lbl in item.get('labels', []) if isinstance(lbl, dict)])
    assignee = (item.get('assignee') or {}).get('login') if isinstance(item.get('assignee'), dict) else None
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument('--state', choices=['open', 'closed', 'all'], default='open', help='Filter by state')
    p.add_argument('--labels', default='', help='Comma-separated label names to filter by')
    p.add_argument('--assignee', default=None, help='Filter by assignee login (or "none")')
    p.add_argument('--creator', default=None, help='Filter by creator login')
    p.add_argument('--include-prs', action='store_true', help='Include pull requests in output (off by default)')
    p.add_argument('--limit', type=int, default=30, help='Maximum number of items to return (default 30)')
    ...
    token = load_token()
    if not token:
        print('[list_issues] Missing GitHub token. Set GITHUB_TOKEN or GITHUB_TOKEN_RO, or add it to .junie/mcp/.env',
              file=sys.stderr)
        return 2
    if len(token) < 20:
        print('[list_issues] Warning: token looks short; you may get 401 Unauthorized.', file=sys.stderr)
