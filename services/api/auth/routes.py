...
async def _get_user_from_any_token(
    ...
    if _token_matches(token, TEST_TOKEN):
        ...
        if not user:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Developer token user not found",
                headers={"WWW-Authenticate": "Bearer"},
            )
        return user
...
async def two_factor_status(current_user: User = Depends(_get_user_from_any_token)):
...
async def setup_two_factor(current_user: User = Depends(_get_user_from_any_token)):
