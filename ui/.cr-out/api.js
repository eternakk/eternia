...
const applyBackoff = async (retryCount) => {
    if (retryCount > 0) {
...
const scheduleSoftReset = (reason) => {
    setTimeout(() => {
...
const waitForOngoingFetch = async () => {
    if (!tokenFetchPromise)
        return false;
...
const fetchWithImmediateRetry = async () => {
    const token = await fetchToken();
    if (token)
        return;
    if (tokenFetchRetries < MAX_TOKEN_FETCH_RETRIES) {
...
const ensureToken = async () => {
    if (TOKEN)
        return;
    if (tryUseStoredToken())
        return;
    try {
        if (await waitForOngoingFetch())
            return;
...
const baseSendCommand = async (a) => {
    await ensureToken();
    return api.post(`/command/${a}`);
};
const baseGetCheckpoints = async () => {
    await ensureToken();
    return api.get("/checkpoints").then(r => r.data);
};
const baseRollbackTo = async (file) => {
    await ensureToken();
    return api.post("/command/rollback", null, { params: { file } });
};
const baseTriggerRitual = async (id) => {
    await ensureToken();
    return api.post(`/api/rituals/trigger/${id}`);
};
const baseSendReward = async (companionName, value) => {
    await ensureToken();
...
export const getState = createSafeApiCall(baseGetState, "Failed to fetch state data");
export const sendCommand = createSafeApiCall(baseSendCommand, "Failed to send command");
export const getCheckpoints = createSafeApiCall(baseGetCheckpoints, "Failed to fetch checkpoints");
export const rollbackTo = createSafeApiCall(baseRollbackTo, "Failed to rollback to checkpoint");
export const triggerRitual = createSafeApiCall(baseTriggerRitual, "Failed to trigger ritual");
export const sendReward = createSafeApiCall(baseSendReward, "Failed to send reward");
// Add function to fetch zones
const baseGetZones = async () => {
    await ensureToken();
    return api.get("/api/zones").then(r => r.data);
};
export const getZones = createSafeApiCall(baseGetZones, "Failed to fetch zones");
const baseGetZoneAssets = async (zoneName) => {
    await ensureToken();
    return api.get("/zone/assets", {
        params: { name: zoneName }
    }).then(r => r.data);
};
export const getZoneAssets = createSafeApiCall(baseGetZoneAssets, "Failed to fetch zone assets");
// Add function to change the current zone
const baseChangeZone = async (zoneName) => {
    await ensureToken();
    return api.post("/api/change_zone", { zone: zoneName });
};
export const changeZone = createSafeApiCall(baseChangeZone, "Failed to change zone");
// Add function to fetch rituals
const baseGetRituals = async () => {
    await ensureToken();
    return api.get("/api/rituals").then(r => r.data);
};
export const getRituals = createSafeApiCall(baseGetRituals, "Failed to fetch rituals");
// Add function to fetch agents
const baseGetAgents = async () => {
    await ensureToken();
    return api.get("/api/agents").then(r => r.data);
};
export const getAgents = createSafeApiCall(baseGetAgents, "Failed to fetch agents");
const baseQuantumQrng = async (n = 128) => {
    await ensureToken();
    return api.post("/api/quantum/qrng", { n }).then(r => r.data);
};
const baseVariationalField = async (seed, size = 32) => {
    await ensureToken();
    return api.post("/api/quantum/variational-field", { seed, size }).then(r => r.data);
};
export const getQuantumBits = createSafeApiCall(baseQuantumQrng, "Failed to get quantum random bits");
export const getVariationalField = createSafeApiCall(baseVariationalField, "Failed to get quantum variational field");
